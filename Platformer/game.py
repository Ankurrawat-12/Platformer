...
class Button:
    def __init__(self, x, y, image):
        img = pygame.transform.scale(image, (tile_size * 2 + 20, (tile_size // 2) + 20))
        self.image = img
...
class Player:
    ...
    def update(self, game_over):
        ...
        if game_over == 0:
            ...
            if world.total_coin == coin_collected:
        ...
        for row in data:
            ...
            for tile in row:
                ...
                if tile == 5:
                    ...
                    platform_group.add(platform)
                if tile == 6:
                    lava = Lava(col_count * tile_size, row_count * tile_size + (tile_size // 2 + 2))
                    lava_group.add(lava)
                if tile == 7:
                    coin = Coin(col_count * tile_size + (tile_size // 2), row_count * tile_size + (tile_size // 2 + 2))
                    coin_group.add(coin)
                    self.total_coin += 1
                if tile == 8:
    ...
    if main_menu:
        ...
        if game_over == 0:
            ...
            if pygame.sprite.spritecollide(player, coin_group, True):
                ...
                coin_collected += 1
                coin_fx.play()
            draw_text(f"X {score}", font_score, white, tile_size - 8, 7)

        blob_group.draw(screen)
        platform_group.draw(screen)
        lava_group.draw(screen)
        coin_group.draw(screen)
        if coin_collected == world.total_coin:
        ...
        if game_over == 1:
            ...
            if level <= max_levels:
                ...
                draw_text("YOU WIN!", font, blue, (screen_width // 2) - 98, screen_height // 2)
                # restart game
                if restart_button.draw():
                    level = 0
                    world_data = []
